      ...
      <form class="border rounded p-4" @submit.prevent="submitForm">
        ...
        <div class="mb-3">
          ...
          <input
            ...
            v-model="user.email"
          />
          <div v-if="showEmailHint" class="form-text text-danger">
            <div v-if="errors.includes('email-invalid')">Invalid email</div>
            <div v-if="errors.includes('email-exists')">Email already exists</div>
          </div>
        </div>
        <div class="mb-3">
          ...
          <div class="position-relative">
            <input
              ...
              @input="validadePassword"
              ref="pwInput"
              v-model="user.password"
            />
            <div class="form-text" v-if="showPwHint">
              <ul>
                ...
                <li
                  :class="errors.includes('password-no-uppercase') ? 'text-danger' : 'text-success'"
                >
                  At least 1 uppercase letter
                </li>
                <li
                  :class="errors.includes('password-no-lowercase') ? 'text-danger' : 'text-success'"
                >
                  At least 1 lowercase letter
                </li>
                <li
                  :class="errors.includes('password-no-special') ? 'text-danger' : 'text-success'"
                >
